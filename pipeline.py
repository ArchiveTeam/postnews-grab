...
LUA_SHA1 = get_hash(os.path.join(CWD, 'postnews.lua'))
    ...
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'profile':
                wget_args.extend(['--warc-header', 'post-news-profile: '+item_value])
                wget_args.append('https://n1nzo2oxji.execute-api.us-east-1.amazonaws.com/prod/public/profiles/@'+item_value.lower())
            elif item_type == 'post':
                wget_args.extend(['--warc-header', 'post-news-post-id: '+item_value])
                wget_args.append('https://n1nzo2oxji.execute-api.us-east-1.amazonaws.com/prod/public/posts/'+item_value)
            elif item_type == 'widget':
                wget_args.extend(['--warc-header', 'post-news-widget-id: '+item_value])
                wget_args.append('https://n1nzo2oxji.execute-api.us-east-1.amazonaws.com/prod/public/widgets?widgetIds='+item_value)
            elif item_type == 'image':
                wget_args.extend(['--warc-header', 'post-news-image: '+item_value])
                wget_args.append('https://d2pmpprut4wwdv.cloudfront.net/images/'+item_value)
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/1/14/Post.news-icon.png" height="50px" title=""/>
        <h2>Post.news <span class="links"><a href="https://post.news/">Website</a> &middot; <a href="http://tracker.archiveteam.org/postnews/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Post.news">Wiki</a></span></h2>
        <p>Archiving Post.ews.</p>
    '''
)
